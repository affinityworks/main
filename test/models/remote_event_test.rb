...
class RemoteEventTest < ActiveSupport::TestCase
  test "find for_event" do
    not_associated = remote_events(:not_associated)
    found_fb_event = FacebookEvent.find_or_initialize_for_event(events(:one).id, not_associated)
    assert_equal not_associated.id, found_fb_event.id
